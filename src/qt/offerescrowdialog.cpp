	...
	ui(new Ui::OfferEscrowDialog)
{
	...
	ui->payMessage->setText(tr("<p>You've created an escrow for %1 of '%2' for %3 %4 (includes a 0.05% escrow arbiter fee which is returned to you if the arbiter is not involved in the escrow)</p><p><FONT COLOR='green'><b>Your payment is in escrow!</b></FONT></p><p>The merchant and arbiter have been sent an escrow notification. The merchant may follow-up with further information.</p><br>").arg(quantity).arg(title).arg(price).arg(currency));			
