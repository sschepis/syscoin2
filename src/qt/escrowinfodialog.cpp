...
bool EscrowInfoDialog::lookup()
{
    ...
    try {
		...
		if (result.type() == UniValue::VOBJ)
		{
			...
			ui->priceEdit->setText(QString("%1 SYS").arg(QString::number(find_value(result.get_obj(), "systotal").get_real())));
			ui->feeEdit->setText(QString("%1 SYS").arg(QString::number(find_value(result.get_obj(), "sysfee").get_real())));
